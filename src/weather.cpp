...
{
    return g->m.is_outside( g->u.posx(), g->u.posy() ) && g->m.get_levz() >= 0;
}
                             ...
                             const tripoint &location )
{
    ...
    for( time_point i = start; i < end; i += 1_hours ) {
        ...
        double temperature = ( location.z >= 0 ? w.temperature : g->get_temperature( location ) ) +
                             ( g->new_game ? 0 : g->m.temperature( g->m.getlocal( location ) ) );
...
void weather_effect::thunder()
{
    ...
    if( !g->u.has_effect( effect_sleep ) && !g->u.is_deaf() && one_in( THUNDER_CHANCE ) ) {
        ...
        } else if( g->u.has_trait( trait_GOODHEARING ) && one_in( 1 - 2 * g->m.get_levz() ) ) {
            add_msg( _( "You hear a rumble of thunder from above." ) );
            sfx::play_variant_sound( "environment", "thunder_far", 100, rng( 0, 359 ) );
        } else if( !g->u.has_trait( trait_BADHEARING ) && one_in( 1 - 3 * g->m.get_levz() ) ) {
            add_msg( _( "You hear a rumble of thunder from above." ) );
