...
std::string item::info( std::vector<iteminfo> &info, const iteminfo_query *parts, int batch ) const
{
    ...
    if( !is_gun() ) {
        if( ammo_data() && parts->test( iteminfo_parts::AMMO_REMAINING_OR_TYPES ) ) {
            ...
            if( !ammo.damage.empty() ) {
                if( parts->test( iteminfo_parts::AMMO_DAMAGE_VALUE ) ) {
                    info.emplace_back( "AMMO", _( "<bold>Damage</bold>: " ), "", ammo.damage.total_damage(), true, "",
                                       false, false );
                }
                if( parts->test( iteminfo_parts::AMMO_DAMAGE_AP ) ) {
                    info.emplace_back( "AMMO", space + _( "Armor-pierce: " ), "", get_ranged_pierce( ammo ), true, "",
                                       true, false );
                }
                ...
                if( parts->test( iteminfo_parts::AMMO_DAMAGE_DISPERSION ) ) {
                    info.emplace_back( "AMMO", space + _( "Dispersion: " ), "", ammo.dispersion, true, "", true,
                                       true );
                }
        ...
        if( mod->magazine_integral() || mod->magazine_current() ) {
            if( mod->magazine_current() && parts->test( iteminfo_parts::GUN_MAGAZINE ) ) {
                info.emplace_back( "GUN", _( "Magazine: " ), string_format( "<stat>%s</stat>",
                                   mod->magazine_current()->tname().c_str() ) );
            }
        ...
        if( parts->test( iteminfo_parts::GUN_AIMING_STATS ) ) {
            info.emplace_back( "GUN", _( "Base aim speed: " ), "<num>", g->u.aim_per_move( *mod, MAX_RECOIL ),
                               true, "", true, true );
        ...
        if( parts->test( iteminfo_parts::GUN_DAMAGE ) ) {
            info.push_back( iteminfo( "GUN", _( "Damage: " ), "", mod->gun_damage( false ).total_damage(), true,
                                      "", false, false ) );
        }
    ...
    if( is_gunmod() ) {
        ...
        if( mod.handling != 0 && parts->test( iteminfo_parts::GUNMOD_HANDLING ) ) {
            info.emplace_back( "GUNMOD", _( "Handling modifier: " ), mod.handling > 0 ? "+" : "", mod.handling,
                               true );
        }
    ...
    if( is_armor() ) {
        ...
        if( parts->test( iteminfo_parts::ARMOR_COVERAGE ) ) {
            info.push_back( iteminfo( "ARMOR", _( "Coverage: " ), "<num>%", get_coverage(), true, "",
                                      false ) );
        }
...
void item::on_wear( Character &p )
{
    ...
    if( &p == &g->u && type->artifact ) {
        add_artifact_messages( type->artifact->effects_worn );
    }
...
void item::on_wield( player &p, int mv )
{
    // TODO: artifacts currently only work with the player character
    if( &p == &g->u && type->artifact ) {
        add_artifact_messages( type->artifact->effects_wielded );
    }
...
void item::on_pickup( Character &p )
{
    ...
    // TODO: artifacts currently only work with the player character
    if( &p == &g->u && type->artifact ) {
        add_artifact_messages( type->artifact->effects_carried );
    }
...
void item::add_artifact_messages( const std::vector<art_effect_passive> &effects )
{
    int net_str = 0;
    int net_dex = 0;
    int net_per = 0;
...
void item::process_artifact( player &p, const tripoint & /*pos*/ )
{
    if( !is_artifact() ) {
        return;
    }
    ...
    const bool worn = p.is_worn( *this );
    const bool wielded = ( this == &g->u.weapon );
    std::vector<art_effect_passive> effects;
    effects = type->artifact->effects_carried;
    if( worn ) {
    ...
    if( is_tool() ) {
        // Recharge this if necessary
        if( ammo_remaining() < ammo_capacity() && calendar::once_every( 1_minutes ) ) {
            //Before incrementing charge, check that any extra requirements are met
            if( check_art_charge_req( *this ) ) {
                switch( type->artifact->charge_type ) {
                    case ARTC_NULL:
                    case NUM_ARTCS:
                        break; // dummy entries
                    case ARTC_TIME:
                    ...
                    case ARTC_FATIGUE:
                        if( calendar::once_every( 1_minutes ) ) {
                            ...
                            p.mod_fatigue( 3 * rng( 1, 3 ) );
                            p.mod_stat( "stamina", -9 * rng( 1, 3 ) * rng( 1, 3 ) * rng( 2, 3 ) );
                            charges++;
                        }
                        break;
                }
        ...
        switch( i ) {
            ...
            case AEP_SMOKE:
                if( one_in( 10 ) ) {
                    tripoint pt( p.posx() + rng( -1, 1 ),
                                 p.posy() + rng( -1, 1 ),
                                 p.posz() );
                    g->m.add_field( pt, fd_smoke, rng( 1, 3 ) );
                }
                break;
            ...
            case AEP_EXTINGUISH:
                for( const tripoint &dest : g->m.points_in_radius( p.pos(), 1 ) ) {
                    g->m.adjust_field_age( dest, fd_fire, -1_turns );
                }
                break;
    ...
    // Recalculate, as this might have changed (by mod_*_bonus above)
    p.str_cur = p.get_str();
    p.int_cur = p.get_int();
    p.dex_cur = p.get_dex();
    p.per_cur = p.get_per();
...
bool item::process_cable( player *p, const tripoint &pos )
{
    ...
    if( !g->m.veh_at( source ) || ( source.z != g->m.get_levz() && !g->m.has_zlevels() ) ) {
