...
bool veh_interact::can_install_part()
{
    ...
    msg << string_format(
            _( "> <color_%1$s>1 tool with %2$s %3$i</color> <color_white>OR</color> <color_%4$s>strength %5$i</color>" ),
            status_color( use_aid ), qual.obj().name.c_str(), lvl,
            status_color( use_str ), str ) << "\n";

    std::string install_color = string_format( "<color_%1$s>",  status_color( ok ||
                                g->u.has_trait( trait_DEBUG_HS ) ) );
    ...
    for( auto &pt : veh->parts ) {
        if( pt.is_engine() && !pt.is_broken() ) {
            ...
            opts.emplace_back( "ENGINE", &pt, action && enable &&
                               enable( pt ) ? hotkey++ : '\0', details, msg );
        }
    }
    ...
    for( auto &pt : veh->parts ) {
        if( pt.is_reactor() && !pt.is_broken() ) {
            opts.emplace_back( "REACTOR", &pt, action && enable &&
                               enable( pt ) ? hotkey++ : '\0', details_ammo );
        }
    }
...
bool veh_interact::can_remove_part( int idx )
{
    ...
    if( !veh->can_unmount( idx ) ) {
        msg << string_format( _( "> <color_%1$s>%2$s</color>" ), status_color( false ),
                              _( "Remove attached parts first" ) ) << "\n";
        ok = false;
    }
...
bool veh_interact::do_tirechange( std::string &msg )
{
    switch( cant_do( 'c' ) ) {
        ...
        case LACK_TOOLS:
            msg = string_format(
                      _( "To change a wheel you need a <color_%1$s>wrench</color>, a <color_%2$s>wheel</color>, and either "
...
void veh_interact::display_details( const vpart_info *part )
{
    ...
    if( part->power != 0 ) {
        fold_and_print( w_details, line + 4, col_2, column_width, c_white,
                        _( "Power: <color_light_gray>%d</color>" ), part->power );
    }
