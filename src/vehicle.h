...
struct vehicle_part {
    ...
    public:
        ...
        bool inside      =
            false;     // if tile provides cover. WARNING: do not read it directly, use vpart_position::is_inside() instead
        bool removed     =
            false;     // true if this part is removed. The part won't disappear until the end of the turn
...
class vehicle
{
    ...
    public:
        ...
        fuel_remainder; // After fuel consumption, this tracks the remainder of fuel < 1, and applies it the next time.
        ...
        int turn_dir =
            0;       // direction, to which vehicle is turning (player control). will rotate frame on next move
        ...
        int tracking_epower     =
            0; // total power consumed by tracking devices (why would you use more than one?)
        int alarm_epower        = 0;
        ...
        bool falling                    =
            false; // Is the vehicle hanging in the air and expected to fall down in the next turn?
