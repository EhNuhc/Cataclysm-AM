...
{
    if( !is_engine_type( e, fuel_type_diesel ) ) {
        return 0.0;
    }
        ...
        bool enabled )
{
    ...
    for( auto &e : parts ) {
        ...
        if( !e.removed && ( !enabled || e.enabled ) && !e.is_broken() && ( flag.empty() ||
...
void vehicle::noise_and_smoke( double load, double time )
{
    ...
    for( size_t p = 0; p < parts.size(); p++ ) {
        if( part_flag( p, "MUFFLER" ) ) {
            m = 1.0 - ( 1.0 - part_info( p ).bonus / 100.0 ) * double( parts[p].hp() ) / part_info(
                    p ).durability;
            if( m < muffle ) {
                muffle = m;
                exhaust_part = int( p );
...
bool vehicle::explode_fuel( int p, damage_type type )
{
    ...
    const int pow = 120 * ( 1 - exp( data.explosion_factor / -5000 * ( parts[p].ammo_remaining() *
                                     data.fuel_size_factor ) ) );
