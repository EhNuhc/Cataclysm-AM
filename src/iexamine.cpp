...
class atm_menu
{
    ...
    private:
        void add_choice( int const i, char const *const title ) {
            amenu.addentry( i, true, -1, title );
        }
        void add_info( int const i, char const *const title ) {
...
void iexamine::rubble( player &p, const tripoint &examp )
{
    ...
    auto it = std::max_element( shovels.begin(), shovels.end(), []( const item * lhs,
    const item * rhs ) {
...
void iexamine::keg( player &p, const tripoint &examp )
{
    ...
    if( !liquid_present ) {
        ...
        if( drinks_inv.empty() ) {
            add_msg( m_info, _( "You don't have any drinks to fill the %s with." ),
 ...
 */
units::mass sum_up_item_weight_by_material( map_stack &stack, const material_id &material,
        bool remove_items, bool include_contents )
{
...
void iexamine::recycler( player &p, const tripoint &examp )
{
    ...
    units::mass steel_weight = sum_up_item_weight_by_material( items_on_map, material_id( "steel" ),
                               false, false );
    if( steel_weight == 0 ) {
        add_msg( m_info,
