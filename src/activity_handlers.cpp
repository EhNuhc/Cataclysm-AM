...
void activity_handlers::burrow_finish( player_activity *act, player *p )
{
    ...
    } else if( g->m.move_cost( pos ) == 2 && g->m.get_levz() == 0 &&
...
void butchery_drops_hardcoded( const mtype *corpse, player *p, const time_point &age,
                               const std::function<int()> &roll_butchery )
{
...
void butchery_drops_harvest( const mtype &mt, player &p, const time_point &age,
                             const std::function<int()> &roll_butchery )
{
    ...
    try {
        ...
        switch( source_type ) {
            case LST_VEHICLE:
                ...
                liquid = item( act.str_values.at( 0 ), calendar::turn,
                               source_veh->fuel_left( act.str_values.at( 0 ) ) );
                break;
...
{
    if( act->index == 0 ) {
        g->m.build_map_cache( g->m.get_levz() );
        g->plfire();
    }
}
