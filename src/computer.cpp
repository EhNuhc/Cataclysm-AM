...
void computer::activate_function( computer_action action )
{
    ...
    switch( action ) {
        ...
        case COMPACT_RESEARCH: {
            ...
            const int seed = g->m.get_levx() + g->m.get_levy() + g->m.get_levz() + alerts;
        ...
        case COMPACT_MISS_LAUNCH: {
            ...
            g->explosion( tripoint( g->u.posx() + 10, g->u.posx() + 21, g->m.get_levz() ), 200, 0.7,
                          true ); //Only explode once. But make it large.

            //...ERASE MISSILE, OPEN SILO, DISABLE COMPUTER
            // For each level between here and the surface, remove the missile
            for( int level = g->m.get_levz(); level <= 0; level++ ) {
                map tmpmap;
                tmpmap.load( g->m.get_levx(), g->m.get_levy(), level, false );
        ...
        case COMPACT_AMIGARA_LOG: // TODO: This is static, move to data file?
            g->u.moves -= 30;
            reset_terminal();
            print_line( _( "NEPower Mine(%d:%d) Log" ), g->m.get_levx(), g->m.get_levy() );
            print_line( _( "\
ENTRY 47:\n\
...
PERTINANT FOREMAN LOGS WILL BE PREPENDED TO NOTES" ),
                        g->m.get_levx(), g->m.get_levy(), abs( g->m.get_levz() ) );
            print_line( _( "\n\
...
void computer::activate_failure( computer_failure_type fail )
{
    ...
    switch( fail ) {
        ...
        case COMPFAIL_ALARM:
            ...
            if( g->m.get_levz() > 0 && !g->events.queued( EVENT_WANTED ) ) {
        ...
        case COMPFAIL_AMIGARA:
            ...
            g->u.add_effect( effect_amigara, 2_minutes );
            g->explosion( tripoint( rng( 0, SEEX * MAPSIZE ), rng( 0, SEEY * MAPSIZE ), g->m.get_levz() ), 10,
                          0.7, false, 10 );
            g->explosion( tripoint( rng( 0, SEEX * MAPSIZE ), rng( 0, SEEY * MAPSIZE ), g->m.get_levz() ), 10,
                          0.7, false, 10 );
            remove_option( COMPACT_AMIGARA_START );
