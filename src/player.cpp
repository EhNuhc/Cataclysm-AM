...
void player::recalc_speed_bonus()
{
    ...
    if( g != nullptr ) {
        ...
        if( has_trait( trait_COLDBLOOD4 ) || ( has_trait( trait_COLDBLOOD3 ) &&
                                               g->get_temperature( pos() ) < 65 ) ) {
...
int player::intimidation() const
{
    ...
    if( has_trait( trait_SAPIOVORE ) ) {
        ...
        ret -= 1;
    } else if( has_trait( trait_BEAUTIFUL ) || has_trait( trait_BEAUTIFUL2 ) ||
               has_trait( trait_BEAUTIFUL3 ) ) {
        ret -= 4;
    }
    if( stim > 20 ) {
        ret += 2;
...
void player::on_hurt( Creature *source, bool disturb /*= true*/ )
{
    ...
    if( disturb ) {
        ...
        if( !is_npc() ) {
            if( source != nullptr ) {
                g->cancel_activity_query( string_format( _( "You were attacked by %s!" ),
                                          source->disp_name().c_str() ) );
            } else {
        ...
        if( tdata.powered && tdata.charge > 0 ) {
            ...
            if( mdata.hunger ) {
                ...
                if( get_hunger() >= 700 ) { // Well into Famished
                    add_msg_if_player( m_warning, _( "You're too famished to keep your %s going." ),
                                       mdata.name.c_str() );
                    tdata.powered = false;
                }
            }
            if( mdata.thirst ) {
                ...
                if( get_thirst() >= 260 ) { // Well into Dehydrated
                    add_msg_if_player( m_warning, _( "You're too dehydrated to keep your %s going." ),
                                       mdata.name.c_str() );
                    tdata.powered = false;
                }
            }
            if( mdata.fatigue ) {
                ...
                if( get_fatigue() >= EXHAUSTED ) { // Exhausted
                    add_msg_if_player( m_warning, _( "You're too exhausted to keep your %s going." ),
                                       mdata.name.c_str() );
                    tdata.powered = false;
                }
            }
...
bool player::pick_style() // Style selection menu
{
    ...
    for( size_t i = 0; i < selectable_styles.size(); i++ ) {
        ...
        kmenu.addentry_desc( i + STYLE_OFFSET, true, -1, _( style.name.c_str() ),
                             _( style.description.c_str() ) );
    }
...
bool player::invoke_item( item *used, const tripoint &pt )
{
    ...
    umenu.entries.end(), []( const uimenu_entry & elem ) {
...
bool player::read( int inventory_position, const bool continuous )
{
    ...
    for( player *elem : apply_morale ) {
        ...
        if( ( elem->has_trait( trait_CANNIBAL ) || elem->has_trait( trait_PSYCHOPATH ) ||
            ...
            elem->add_morale( MORALE_BOOK, 0, type->fun * 15, decay_start + 3_minutes, decay_start, false,
                              it.type );
        }
    }
    ...
    for( auto &elem : learners ) {
        ...
        if( reading->fun != 0 ) {
            ...
            if( ( learner->has_trait( trait_CANNIBAL ) || learner->has_trait( trait_PSYCHOPATH ) ||
                ...
                learner->add_morale( MORALE_BOOK, fun_bonus, reading->fun * 15, 6_minutes, 3_minutes, true,
                                     book.type );
            }
        }
