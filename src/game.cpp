...
game::game() :
    ...
    faction_manager_ptr( new faction_manager() ),
...
bool game::start_game()
{
    ...
    load_master();
    u.setID(); // should be as soon as possible, but *after* load_master
    ...
    const bool spawn_near =
        get_option<bool>( "BLACK_ROAD" ) || scen->has_flag( "SUR_START" );
...
void game::load_npcs()
{
    ...
    for( const auto &temp : overmap_buffer.get_npcs_near_player( radius ) ) {
        ...
        if( sm_loc.x < m.get_levx() || sm_loc.x >= m.get_levx() + MAPSIZE ||
            sm_loc.y < m.get_levy() || sm_loc.y >= m.get_levy() + MAPSIZE ||
            ( sm_loc.z != m.get_levz() && !m.has_zlevels() ) ) {
            continue;
        }

        add_msg( m_debug, "game::load_npcs: Spawning static NPC, %d:%d:%d (%d:%d:%d)",
                 m.get_levx(), m.get_levy(), m.get_levz(), sm_loc.x, sm_loc.y, sm_loc.z );
        temp->place_on_map();
...
void game::create_starting_npcs()
{
    ...
    tmp->spawn_at_precise( { m.get_levx(), m.get_levy() }, u.pos() - point( 1, 1 ) );
...
bool game::do_turn()
{
    ...
    for( auto &elem : MAPBUFFER ) {
        ...
        const bool in_bubble_z = m.has_zlevels() || sm_loc.z == m.get_levz();
    ...
    m.build_map_cache( m.get_levz(), true );
    monmove();
    ...
    if( m.get_levz() >= 0 && !u.is_underwater() ) {
...
void game::update_weather()
{
    if( weather == WEATHER_NULL || calendar::turn >= nextweather ) {
        ...
        if( weather != old_weather && weather_data( weather ).dangerous &&
            m.get_levz() >= 0 && m.is_outside( u.pos() )
            && !u.has_activity( activity_id( "ACT_WAIT_WEATHER" ) ) ) {
            cancel_activity_query( string_format( _( "The weather changed to %s!" ),
...
int game::get_temperature( const tripoint &location )
{
    ...
    return ( location.z < 0 ? AVERAGE_ANNUAL_TEMPERATURE : temperature ) +
           ( new_game ? 0 : m.temperature( location ) );
}
                ...
                */
                ...
                if( !use_tiles ) {
                    ...
                    for( auto &elem : wPrint.vdrops ) {
                        const tripoint location( elem.first + offset_x, elem.second + offset_y, m.get_levz() );
            ...
            if( uquit != QUIT_WATCH && get_option<bool>( "ANIMATION_SCT" ) ) {
                    ...
                    for( auto &elem : SCT.vSCT ) {
                        ...
                        if( elem.getStep() > 0 ) {
                            for( size_t i = 0; i < elem.getText().length(); ++i ) {
                                const tripoint location( elem.getPosX() + i, elem.getPosY(), m.get_levz() );
                                const lit_level lighting = visibility_cache[location.x][location.y];
                                wmove( w_terrain, location.y - offset_y, location.x - offset_x );
                                if( !m.apply_vision_effects( w_terrain, lighting, cache ) ) {
                                    m.drawsq( w_terrain, u, location, false, true,
                ...
                for( auto iter = SCT.vSCT.rbegin(); iter != SCT.vSCT.rend(); ++iter ) {
                    ...
                    for( int i = 0; i < ( int )iter->getText().length(); ++i ) {
                        tripoint tmp( iter->getPosX() + i, iter->getPosY(), m.get_levz() );
...
// TODO: This is positively horrifying. Kill it with fire and blade.
...
void game::debug()
{
    ...
    switch( action ) {
        ...
        case 5: {
            ...
            temp->normalize();
            temp->randomize();
            temp->spawn_at_precise( { m.get_levx(), m.get_levy() }, u.pos() + point( -4, -4 ) );
        ...
        case 7: {
            ...
            popup_top(
                s.c_str(),
                u.posx(), u.posy(), m.get_levx(), m.get_levy(),
...
void game::draw_sidebar()
{
    ...
    const auto ll = get_light_level( u.fine_detail_vision_mod() );
...
void game::draw_ter( const tripoint &center, const bool looking, const bool draw_sounds )
{
    ...
    wmove( w_terrain, POSY + u.pos().y - center.y, POSX + u.pos().x - center.x );
}
...
{
    const int minz = m.has_zlevels() ? -OVERMAP_DEPTH : m.get_levz();
    const int maxz = m.has_zlevels() ? OVERMAP_HEIGHT : m.get_levz();
...
void game::draw_minimap()
{
    ...
    for( int i = -2; i <= 2; i++ ) {
        for( int j = -2; j <= 2; j++ ) {
            ...
            nc_color ter_color;
            long ter_sym;
            const bool seen = overmap_buffer.seen( omx, omy, m.get_levz() );
            const bool vehicle_here = overmap_buffer.has_vehicle( omx, omy, m.get_levz() );
            if( overmap_buffer.has_note( omx, omy, m.get_levz() ) ) {

                const std::string &note_text = overmap_buffer.note( omx, omy, m.get_levz() );
                ...
                const oter_id &cur_ter = overmap_buffer.ter( omx, omy, m.get_levz() );
                ter_sym = cur_ter->get_sym();
                if( overmap_buffer.is_explored( omx, omy, m.get_levz() ) ) {
                    ter_color = c_dark_gray;
                } else {
    ...
    const int sight_points = u.overmap_sight_range( light_level( u.posz() ) );
    for( int i = -3; i <= 3; i++ ) {
        for( int j = -3; j <= 3; j++ ) {
            if( i > -3 && i < 3 && j > -3 && j < 3 ) {
            ...
            const int omx = cursx + i;
            const int omy = cursy + j;
            tripoint const cur_pos {omx, omy, m.get_levz()};
            if( overmap_buffer.get_horde_size( omx, omy, m.get_levz() ) >= HORDE_VISIBILITY_SIZE ) {
                tripoint const cur_pos {omx, omy, m.get_levz()};
                if( overmap_buffer.seen( omx, omy, m.get_levz() )
                    && u.overmap_los( cur_pos, sight_points ) ) {
                    mvwputch( w_minimap, j + 3, i + 3, c_green,
                              overmap_buffer.get_horde_size( omx, omy, m.get_levz() ) > HORDE_VISIBILITY_SIZE * 2 ? 'Z' : 'z' );
                }
            }
        }
    }
...
bool game::swap_critters( Creature &a, Creature &b )
{
    ...
    if( m.veh_at( u_or_npc->pos() ).part_with_feature( VPFLAG_BOARDABLE ) ) {
        m.board_vehicle( u_or_npc->pos(), u_or_npc );
    }

    if( m.veh_at( other_npc->pos() ).part_with_feature( VPFLAG_BOARDABLE ) ) {
                            ...
                            bool has_first_point, bool select_zone )
{
    ...
    do {
        ...
        if( action == "LIST_ITEMS" ) {
            ...
            add_msg( m_debug, "levx: %d, levy: %d, levz :%d", m.get_levx(), m.get_levy(), new_levz );
                          ...
                          const monster *const source_mon )
{
    ...
    for( const auto &e : m.points_in_radius( u.pos(), 1 ) ) {
        auto veh = veh_pointer_or_null( m.veh_at( e ) );
    ...
    switch( u.rate_action_reload( *it ) ) {
        case HINT_IFFY:
            ...
            if( it->is_ammo_belt() ) {
                ...
                if( linkage != "NULL" && !u.has_charges( linkage, 1 ) ) {
...
void game::mend( int pos )
{
    ...
    item &obj = u.i_at( pos );
    if( u.has_item( obj ) ) {
        u.mend_item( item_location( u, &obj ) );
    }
}
...
void game::place_player( const tripoint &dest_loc )
{
    ...
    if( m.has_zlevels() && dest_loc.z != m.get_levz() ) {
    ...
    if( !m.has_flag( "SEALED", u.pos() ) ) {
        if( get_option<bool>( "NO_AUTO_PICKUP_ZONES_LIST_ITEMS" ) ||
            !zone_manager::get_manager().has( zone_type_id( "NO_AUTO_PICKUP" ), u.pos() ) ) {
...
void game::vertical_move( int movez, bool force )
{
    ...
    // Because m.get_levz takes z-value from the map, it will change when vertical_shift (m.has_zlevels() == true)
    ...
    const int z_before = m.get_levz();
    const int z_after = m.get_levz() + movez;
    ...
    if( m.has_zlevels() ) {
        ...
        maybetmp.load( m.get_levx(), m.get_levy(), z_after, false );
    }
...
tripoint game::find_or_make_stairs( map &mp, const int z_after, bool &rope_ladder )
{
    ...
    int best = INT_MAX;
    const int movez = z_after - m.get_levz();
...
void game::vertical_shift( const int z_after )
{
    ...
    if( !m.has_zlevels() ) {
        ...
        m.load( m.get_levx(), m.get_levy(), z_after, true );
...
void game::update_map( int &x, int &y )
{
    ...
    if( shiftx == 0 && shifty == 0 ) {
        ...
        u.setpos( tripoint( x, y, m.get_levz() ) );
    ...
    // m.get_levz() should later be removed, when there is no longer such a thing
    // as "current z-level"
    u.setpos( tripoint( x, y, m.get_levz() ) );
...
void game::replace_stair_monsters()
{
    for( auto &elem : coming_to_stairs ) {
        elem.staircount = 0;
        tripoint spawn_point( elem.posx(), elem.posy(), m.get_levz() );
...
void game::update_stair_monsters()
{
    ...
    const bool from_below = monstairz < m.get_levz();
    ...
    for( size_t i = 0; i < coming_to_stairs.size(); i++ ) {
        ...
        const tripoint dest{mposx, mposy, m.get_levz()};
        ...
        if( is_empty( dest ) ) {
            ...
            while( tries < creature_push_attempts ) {
                ...
                tripoint pos( iposx, iposy, m.get_levz() );
            ...
            while( tries < creature_push_attempts ) {
                ...
                if( !critter_at( pos ) && other.can_move_to( pos ) ) {
                    other.setpos( tripoint( iposx, iposy, m.get_levz() ) );
                    other.moves -= 50;
                    std::string msg;
...
void game::perhaps_add_random_npc()
{
    ...
    if( !get_option<bool>( "RANDOM_NPC" ) || ( !m.has_zlevels() && m.get_levz() != 0 ) ) {
        return;
    }
...
void game::add_artifact_dreams( )
{
    ...
    std::list<item *> art_items = u.get_artifact_items();
