...
class Font
{
    public:
        ...
        virtual void OutputChar( std::string ch, int x, int y, unsigned char color,
                                 cata_cursesport::font_style FS ) = 0;
        virtual void draw_ascii_lines( unsigned char line_id, int drawx, int drawy, int FG,
                                       cata_cursesport::font_style FS ) const;
...
class CachedTTFFont : public Font
{
    public:
        ...
        virtual void OutputChar( std::string ch, int x, int y, unsigned char color,
                                 cata_cursesport::font_style FS ) override;
    protected:
        ...
        struct key_t {
            ...
            bool operator<( key_t const &rhs ) const noexcept {
                return ( FS.to_ulong() == rhs.FS.to_ulong() ) ? ( ( color == rhs.color ) ? codepoints <
                        rhs.codepoints : color < rhs.color ) : FS.to_ulong() < rhs.FS.to_ulong();
            }
        };
...
class BitmapFont : public Font
{
    public:
        ...
        virtual void draw_ascii_lines( unsigned char line_id, int drawx, int drawy, int FG,
                                       cata_cursesport::font_style FS ) const override;
    protected:
...
bool InitSDL()
{
    ...
    if( ( ret & IMG_INIT_PNG ) != IMG_INIT_PNG ) {
        dbg( D_ERROR ) << "IMG_Init failed to initialize PNG support, tiles won't work, error: " <<
                       IMG_GetError();
...
bool SetupRenderTarget()
{
    if( SDL_SetRenderDrawBlendMode( renderer.get(), SDL_BLENDMODE_NONE ) != 0 ) {
        dbg( D_ERROR ) << "SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_NONE) failed: " <<
                       SDL_GetError();
...
bool WinCreate()
{
    ...
    if( !software_renderer ) {
        ...
        if( !renderer ) {
            dbg( D_ERROR ) << "Failed to initialize accelerated renderer, falling back to software rendering: "
                           << SDL_GetError();
...
void Font::draw_ascii_lines( unsigned char line_id, int drawx, int drawy, int FG,
                             cata_cursesport::font_style FS ) const
{
    ( void ) FS; // unused
    switch( line_id ) {
...
bool input_context::get_coordinates( const catacurses::window &capture_win_, int &x, int &y )
{
    ...
    if( tile_iso && use_tiles ) {
        const int screen_column = round( ( float )( coordinate_x - win_left - ( (
                                             win_right - win_left ) / 2 + win_left ) ) / ( fw / 2 ) );
        const int screen_row = round( ( float )( coordinate_y - win_top - ( win_bottom - win_top ) / 2 +
                                      win_top ) / ( fw / 4 ) );
...
void BitmapFont::draw_ascii_lines( unsigned char line_id, int drawx, int drawy, int FG,
                                   cata_cursesport::font_style FS ) const
{
    ( void ) FS; // unused
...
CachedTTFFont::CachedTTFFont( const int w, const int h, std::string _typeface, int _fontsize,
                              const bool _fontblending )
    : Font( w, h )
...
void play_music_file( const std::string &filename, int volume )
{
    ...
    Mix_VolumeMusic( get_option<bool>( "SOUND_ENABLED" ) ? volume * get_option<int>( "MUSIC_VOLUME" ) /
                     100 : 0 );
