...
void npc::place_on_map()
{
    ...
    const int dmx = submap_coords.x - g->m.get_levx();
    const int dmy = submap_coords.y - g->m.get_levy();
...
void npc::setpos( const tripoint &pos )
{
    ...
    submap_coords.x = g->m.get_levx() + pos.x / SEEX;
    submap_coords.y = g->m.get_levy() + pos.y / SEEY;
